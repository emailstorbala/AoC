...
class Utilities {
  public:
    ...
    bool StringContainsString(const std::string &inpStr,
    ...
    SplitStringExactHalf(const std::string &inp);
    std::list<std::string> SimpleFileRead(std::string_view _fname);
    std::set<char> getCommonCharacters(std::string_view str1,
                                       std::string_view _str2);
