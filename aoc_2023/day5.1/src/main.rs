...
fn get_result_list(inp_list: &Vec<i64>, line: &str) -> Vec<i64> {
    let mut tmp_list: Vec<i64> = Vec::new();
    let mut common_map: CommonMap = HashMap::new();
    ...
    // Collect soil list
    for src in inp_list {
        let dest: i64 = if common_map.contains_key(&src) {
            *common_map.get(&src).unwrap()
        } else {
            *src
        };
        tmp_list.push(dest);
            ...
            match line {
                ...
                _ => {
                    ...
                    if seed_to_soil_mode {
                        ...
                        tmp_list = get_result_list(&seeds, line);
                    } else if soil_to_fert_mode {
                        println!("Soil->Fertilizer mode started ...");
                        tmp_list = get_result_list(&tmp_list, line);
