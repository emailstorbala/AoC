...
fn collect_numbers(content_map: &HashMap<(usize, usize), char>, num_of_rows: usize) -> HashMap<i32, Vec<(usize, usize)>> {
...
fn check_positions_match(positions: Vec<(usize, usize)>, content_map: &HashMap<(usize, usize), char>) -> bool {
    ...
    for (row, pos) in positions {
        ...
        if content_map.contains_key(&(diag_topleft_row, diag_topleft_pos)) {
            let cell_char: char = *content_map.get(&(diag_topleft_row, diag_topleft_pos)).unwrap();
        ...
        if content_map.contains_key(&(diag_topright_row, diag_topright_pos)) {
            let cell_char: char = *content_map.get(&(diag_topright_row, diag_topright_pos)).unwrap();
        ...
        if content_map.contains_key(&(diag_bottomright_row, diag_bottomright_pos)) {
            let cell_char: char = *content_map.get(&(diag_bottomright_row, diag_bottomright_pos)).unwrap();
        ...
        if content_map.contains_key(&(diag_bottomleft_row, diag_bottomleft_pos)) {
            let cell_char: char = *content_map.get(&(diag_bottomleft_row, diag_bottomleft_pos)).unwrap();
...
fn read_contents(content: String) -> i32 {
    ...
    let num_and_positions = collect_numbers(&content_map, row_num);
